        ...
        public void solve(int testNumber, InputReader in, BufferedWriter out) throws IOException {
            while(true) {
                ...
                for(int i = 0; i < n; i++) {
                    char age, way, s, e;
                    int dist;
                    age = in.readString().charAt(0);
                    way = in.readString().charAt(0);
                    s = in.readString().charAt(0);
                    e = in.readString().charAt(0);
                    dist = in.nextInt();
                    if(age == 'Y') {
                        if(way == 'U') {
                            a[s - 'A'][e - 'A'] = Math.min(a[s - 'A'][e - 'A'], dist);
                        } else {
                            a[s - 'A'][e - 'A'] = Math.min(a[s - 'A'][e - 'A'], dist);
                            a[e - 'A'][s - 'A'] = Math.min(a[e - 'A'][s - 'A'], dist);
                        }
                    } else {
                        if(way == 'U') {
                            b[s - 'A'][e - 'A'] = Math.min(b[s - 'A'][e - 'A'], dist);
                        } else {
                            b[s - 'A'][e - 'A'] = Math.min(b[s - 'A'][e - 'A'], dist);
                            b[e - 'A'][s - 'A'] = Math.min(b[e - 'A'][s - 'A'], dist);
                        }
                    }
                }
                ...
                for(int i = 0; i < 26; i++) {
                    int cur = a[s - 'A'][i] + b[e - 'A'][i];
                    if(cur < ans) {
                        ans = cur;
                        ind = new StringBuilder(Character.toString((char)('A' + i)));
                    } else if(cur == ans) {
                        ind = new StringBuilder(ind.toString() + " " + (char)('A' + i));
                    }
                }

                if(ans == inf) {
                    out.write("You will never meet.\n");
                } else {
                    out.write(Integer.toString(ans) + " " + ind + "\n");
                }
